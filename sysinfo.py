...
def get_static_sys_info() -> Dict[str, object]:
    ...
    # for potential multi platform compatibility - kernel version format differs
    ...
    # for potential multi platform compatibility - some systems might not report freq
    ...
    # Disk
    # for potential multi platform compatibility in the future
...
def _af_link_family():
    ...
    # for potential multi platform compatibility - AF_PACKET needed for linux
    ...
    for iface, addr_list in addrs.items():
        ...
        for a in addr_list:
            if a.family == socket.AF_INET:
                ipv4 = a.address
            # for potential multi platform compatibility
            if (AF_LINK and a.family == AF_LINK) or (AF_PACKET and a.family == AF_PACKET):
                mac = a.address
                # break  # could break here but might miss multiple addresses
...
def get_net_io_counters() -> Optional[Tuple[int, int]]:
    ...
    try:
        ...
        # for potential multi platform compatibility - some systems might not have counters
        sent = c.bytes_sent
        recv = c.bytes_recv
        return (sent, recv)
    except Exception:
